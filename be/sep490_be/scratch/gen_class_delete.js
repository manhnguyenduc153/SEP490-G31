...
async function createExcel() {
    ...
    sheet.addRow(['Code Module', 'ClassService', 'Method', 'DeleteAsync', '','','','','','','','']);
    ...
    addSubHeader('Class Existence');
    addCondition('Class exists in DB (ID valid)', ['O', 'O', 'O', '', 'O', '', 'O', 'O', 'O', '']);
    addCondition('Class does not exist (ID not found)', ['', '', '', 'O', '', '', '', '', '', '']);
    addCondition('Class ID <= 0 or missing', ['', '', '', '', '', 'O', '', '', '', '']);
    
    addSubHeader('Class Status');
    addCondition('Class status is Planning', ['O', 'O', 'O', '', '', '', 'O', 'O', 'O', '']);
    addCondition('Class status is NOT Planning (Started)', ['', '', '', '', 'O', '', '', '', '', '']);
    
    addSubHeader('Associations (Students & Registrations)');
    addCondition('Class has no students enrolled', ['O', '', '', '', '', '', '', '', '', '']);
    addCondition('Class has students & Semester found', ['', 'O', '', '', '', '', '', '', '', '']);
    addCondition('Class has students & Semester NOT found', ['', '', 'O', '', '', '', '', '', '', '']);
    addCondition('Database error updating associations', ['', '', '', '', '', '', 'O', 'O', 'O', '']);
    ...
    addCondition('Throws Exception', ['', '', '', '', '', '', 'O', 'O', 'O', 'O']);
    
    addSubHeader('Log message');
    addCondition('DELETE_CLASS_SUCCESS', ['O', 'O', 'O', '', '', '', '', '', '', '']);
    addCondition('ERR_CLASS_NOT_FOUND', ['', '', '', 'O', '', 'O', '', '', '', '']);
    addCondition('ERR_CLASS_ALREADY_STARTED', ['', '', '', '', 'O', '', '', '', '', '']);
    addCondition('Transaction Rollback / Exception', ['', '', '', '', '', '', 'O', 'O', 'O', 'O']);
    ...
    const desktopPath = path.join(os.homedir(), 'Desktop', 'DeleteClassAsync_TestCase.xlsx');
