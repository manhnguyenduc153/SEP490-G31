...
async function createExcel() {
    ...
    sheet.addRow(['Code Module', 'HomeworkService', 'Method', 'GradeSubmissionAsync', '','','','','','','','']);
    ...
    addSubHeader('Submission Existence');
    addCondition('Submission exists in DB (IsDeleted = false)', ['O', 'O', 'O', 'O', 'O', '', '', '', 'O', 'O']);
    addCondition('Submission does not exist (ID not found)', ['', '', '', '', '', 'O', '', '', '', '']);
    addCondition('Submission exists in DB but IsDeleted = true', ['', '', '', '', '', '', 'O', '', '', '']);
    addCondition('Submission ID <= 0 or missing', ['', '', '', '', '', '', '', 'O', '', '']);
    
    addSubHeader('Grading Data (DTO)');
    addCondition('Score < TotalScore (Valid)', ['O', '', 'O', '', '', '', '', '', 'O', '']);
    addCondition('Score == TotalScore (Boundary Valid)', ['', 'O', '', '', '', '', '', '', '', '']);
    addCondition('Score > TotalScore (Invalid)', ['', '', '', 'O', '', '', '', '', '', '']);
    addCondition('Score is negative', ['', '', '', '', 'O', '', '', '', '', '']);
    addCondition('TeacherFeedback is provided', ['O', 'O', '', 'O', 'O', '', '', '', 'O', '']);
    addCondition('TeacherFeedback is empty', ['', '', 'O', '', '', '', '', '', '', '']);
    ...
    addCondition('True (T) / Success', ['O', 'O', 'O', '', '', '', '', '', '', '']);
    addCondition('False (F) / Failed', ['', '', '', 'O', 'O', 'O', 'O', 'O', 'O', 'O']);
    
    addSubHeader('Exception');
    addCondition('Throws Exception', ['', '', '', '', '', '', '', '', 'O', 'O']);
    
    addSubHeader('Log message');
    addCondition('Chấm điểm thành công', ['O', 'O', 'O', '', '', '', '', '', '', '']);
    addCondition('Không tìm thấy bài nộp', ['', '', '', '', '', 'O', 'O', 'O', '', '']);
    addCondition('Điểm không được vượt quá tối đa', ['', '', '', 'O', '', '', '', '', '', '']);
    addCondition('Lỗi kết nối / DB error / Validate', ['', '', '', '', 'O', '', '', '', 'O', 'O']);
    
    let confirmEnd = currentRow - 1;

    // RESULT Section
    let resultStart = currentRow;
    sheet.addRow(['Result', 'Type(N : Normal, A : Abnormal, B : Boundary)', 'N', 'B', 'N', 'A', 'A', 'A', 'A', 'A', 'A', 'A']);
    sheet.getCell('B' + currentRow).font = boldFont;
    currentRow++;
    ...
    const desktopPath = path.join(os.homedir(), 'Desktop', 'GradeSubmissionAsync_TestCase.xlsx');
