...
async function createExcel() {
    ...
    sheet.addRow(['Code Module', 'HomeworkService', 'Method', 'DeleteHomeworkAsync', '','','','','','','','']);
    ...
    addCondition('Homework exists in DB (IsDeleted = false)', ['O', 'O', '', '', '', 'O', 'O', 'O', 'O', 'O']);
    addCondition('Homework does not exist (ID not found)', ['', '', 'O', '', '', '', '', '', '', '']);
    addCondition('Homework exists in DB but IsDeleted = true', ['', '', '', 'O', '', '', '', '', '', '']);
    addCondition('Homework ID <= 0 or missing', ['', '', '', '', 'O', '', '', '', '', '']);
    
    addSubHeader('Homework Status & Associations');
    addCondition('Homework has no submissions (Pending/New)', ['O', '', '', '', '', '', 'O', '', 'O', '']);
    addCondition('Homework already has student submissions', ['', 'O', '', '', '', '', '', '', '', '']);
    addCondition('Homework status is Graded/Closed', ['', '', '', '', '', 'O', '', '', '', '']);
    addCondition('Database constraint error on delete (lock)', ['', '', '', '', '', '', '', 'O', '', '']);
    ...
    addCondition('Throws Exception', ['', '', '', '', '', '', '', 'O', 'O', 'O']);
    
    addSubHeader('Log message');
    addCondition('Xóa bài tập thành công', ['O', 'O', '', '', '', '', 'O', '', '', '']);
    addCondition('Không tìm thấy bài tập', ['', '', 'O', 'O', 'O', '', '', '', '', '']);
    addCondition('Không thể xóa bài tập đã có điểm/đã đóng', ['', '', '', '', '', 'O', '', '', '', '']);
    addCondition('Lỗi validate / DB', ['', '', '', '', '', '', '', 'O', 'O', 'O']);
    
    let confirmEnd = currentRow - 1;

    // RESULT Section
    let resultStart = currentRow;
    sheet.addRow(['Result', 'Type(N : Normal, A : Abnormal, B : Boundary)', 'N', 'A', 'A', 'A', 'A', 'A', 'N', 'A', 'A', 'A']);
    sheet.getCell('B' + currentRow).font = boldFont;
    currentRow++;
    ...
    const desktopPath = path.join(os.homedir(), 'Desktop', 'DeleteHomeworkAsync_TestCase.xlsx');
