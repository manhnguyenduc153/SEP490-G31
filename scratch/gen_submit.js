...
async function createExcel() {
    ...
    sheet.addRow(['Code Module', 'HomeworkService', 'Method', 'SubmitHomeworkAsync', '','','','','','','','']);
    ...
    addSubHeader('Validation (Student & Homework)');
    addCondition('StudentId > 0 (Valid)', ['O', 'O', 'O', 'O', '', 'O', 'O', 'O', 'O', 'O']);
    addCondition('StudentId <= 0 or null', ['', '', '', '', 'O', '', '', '', '', '']);
    addCondition('Homework exists, IsDeleted = false, Status != 0', ['O', 'O', 'O', 'O', '', '', '', 'O', 'O', 'O']);
    addCondition('Homework does not exist or IsDeleted = true', ['', '', '', '', '', 'O', '', '', '', '']);
    addCondition('Homework Status == 0 (Closed)', ['', '', '', '', '', '', 'O', '', '', '']);
    
    addSubHeader('Submission Context (Business Logic)');
    addCondition('First time submission (No existing record)', ['O', 'O', '', '', '', '', '', 'O', '', '']);
    addCondition('Resubmitting (Record already exists)', ['', '', 'O', 'O', '', '', '', '', 'O', '']);
    addCondition('Submit before or on DueDate (On time)', ['O', '', 'O', '', '', '', '', 'O', 'O', '']);
    addCondition('Submit after DueDate (Late)', ['', 'O', '', 'O', '', '', '', '', '', '']);
    ...
    addCondition('True (T) / Success', ['O', 'O', 'O', 'O', '', '', '', 'O', '', '']);
    addCondition('False (F) / Failed', ['', '', '', '', 'O', 'O', 'O', '', 'O', 'O']);
    
    addSubHeader('Exception');
    addCondition('Throws Exception', ['', '', '', '', '', '', '', '', 'O', 'O']);
    
    addSubHeader('Log message');
    addCondition('Nộp bài thành công', ['O', 'O', 'O', 'O', '', '', '', 'O', '', '']);
    addCondition('Khong xac dinh duoc sinh vien', ['', '', '', '', 'O', '', '', '', '', '']);
    addCondition('Bài tập không khả dụng hoặc đã đóng', ['', '', '', '', '', 'O', 'O', '', '', '']);
    addCondition('Lỗi kết nối / DB error', ['', '', '', '', '', '', '', '', 'O', 'O']);
    
    let confirmEnd = currentRow - 1;

    // RESULT Section
    let resultStart = currentRow;
    sheet.addRow(['Result', 'Type(N : Normal, A : Abnormal, B : Boundary)', 'N', 'N', 'N', 'N', 'A', 'A', 'A', 'N', 'A', 'A']);
    sheet.getCell('B' + currentRow).font = boldFont;
    currentRow++;
    ...
    const desktopPath = path.join(os.homedir(), 'Desktop', 'SubmitHomeworkAsync_TestCase.xlsx');
